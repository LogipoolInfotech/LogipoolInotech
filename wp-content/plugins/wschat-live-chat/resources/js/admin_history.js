...
jQuery(document).ready(function() {
    ...
    const wrapper = jQuery('.chat-panel-header');
    ...
    chat.on(EVENTS.WSCHAT_ON_DELETE_CONVERSATIONS, () => {
    ...
    chat.on(EVENTS.WSCHAT_ON_SET_CONVERSATION, (data) => {
    	jQuery('#email_transcript_field_email').val(data.user.meta.email);
        chat_panel.html('');
        if (data.user) {
            ...
            if (row.type === 'info') {
                row_template = `
                  <div class="row g-0 w-100 message-item text-center" data-message-id="{{MESSAGE_ID}}">
                    <div class="text-center p-2 shadow w-50 m-auto rounded">
                        ${row.body.text}
                    <div>
                  </div>
                `;
    ...
    function get_conversion_id_from_url() {
    	const params = new URLSearchParams(location.search);

		return params.get('conversation_id');
    }

    const conversation_id = get_conversion_id_from_url();

    if (conversation_id) {
    	chat.connector.join_conversation(conversation_id);
    }

    setTimeout(() => {
		jQuery('#wschat_email_transcript_form_close').addClass('d-none');
    	jQuery('.wschat_email_transcript_form_popup').removeClass('d-none').addClass('active');
    }, 1000);
});
