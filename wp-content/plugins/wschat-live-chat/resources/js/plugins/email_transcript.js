...
export class EmailTranscriptPlugin {
	...
	show_form() {
		...
		/* These commited lines of codes are related to pre-fil of email field in email transcript
			please ignore these as these are on progress
		*/
		
		// if( this?.chat?.conversation?.user?.meta?.email ) {
		// 	email = this?.chat?.conversation?.user?.meta?.email;
		// }else if(this?.chat?.conversation?.meta?.pre_chat_form?.email?.value){
