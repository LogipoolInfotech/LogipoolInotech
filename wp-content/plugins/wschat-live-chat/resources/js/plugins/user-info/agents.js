        	...
        	<div className="d-flex mb-2 align-items-center gap-3">
            	...
            	<div className="d-flex">
                	{agents
                		...
                		.map((agent , i)  => <AgentAvatar key={agent.ID} agent={agent} conversation={conversation} onDelete ={ () => RemoveAgent(i)} /> )}
            	</div>
        	</div>
...
export const AgentAvatar = props => {
	...
	return (<div
        ...
        <div
            className="ratio ratio-1x1 rounded-circle border border-3 border-primary overflow-hidden shadow-sm 	position-relative">
            <img
                src={props.agent.avatar}
            />
			
        </div>
		<span className="my-icon elex-ws-chat-profile-deassign position-absolute top-0 end-0" onClick={() => deassignAgent(props , props.onDelete )}>&#x2715;</span>
    </div>)
};
...
const  deassignAgent = (props,callback) => {
	...
	jQuery.ajax({
		...
		url: ajaxurl,
		data: {
		action: 'wschat_admin_deAssign_Agent',
		nonce: jQuery('input[name=wschat_settings_nonce]').val(),
		participant_id: props.agent.ID,
